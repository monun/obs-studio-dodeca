...
- 개발·시험용 OBS 설정은 백업 없이 사용한다. 자동화는 작업 공간의 설치본을 명시적인 `--portable`로 실행하며, 일반 모드는 별도 사용자 계정이나 VM에서 검증한다. `SHGetFolderPathW` 후킹만으로 개인 설정과의 격리를 보장하지 않는다.
- PyQt 휠이 번들 Qt와 호환되지 않으면 같은 Qt로 빌드한 임시 검증 모듈을 사용하고, 검사 종료 후 설치본에서 제외한다. 시험 INI의 Windows 경로는 `/`로 기록하고 OBS가 저장한 UTF-8 BOM을 처리한다. OBS 종료 전에 시험 웹소켓 연결을 닫는다.
